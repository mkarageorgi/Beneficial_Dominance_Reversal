...
```{r}
# Specify the doses for which predictions are needed
doses_2 = c(1.5, 2.5, 5, 7.5, 10, 15, 20)

# Create a grid of these specific doses and genotypes for which we want predictions
specific.doses_2 = expand.grid(Dose = doses_2, Genotype = unique(data.resistance$Genotype))

# Predict survival using the resistance data model
predictions.specific.doses.R_2 = predict(model.R, newdata = specific.doses_2, type = "response")

# Attach the predictions to the specific_doses data frame
specific.doses_2$Survival = round(predictions.specific.doses.R_2,2)
...
```{r}
all.metrics.LD50 = all.metrics %>%
  unite(col = "Parameter", Genotype.Allele, Metric, sep = "_") %>%
  rename(LD50 = Estimate) %>%
  
  mutate(LD50.SS = all.metrics %>% filter(Genotype.Allele == "SS" & Metric == "ED50") %>% pull(Estimate)) %>%

  # Then calculate the change
  mutate(change = (LD50 - LD50.SS)/LD50.SS * 100)
```
...
R1_ld50 = ggplot() +
  ...
  geom_richtext(aes(x = 0.7, y = 15, 
                label = "*s* = 0.45 (-0.18, 0.53) <br> *h* = 1.77 (-7432, 298)"), 
...
R2_ld50 = ggplot() +
 ...
 # Add the s and h coefficients for R2
  geom_richtext(aes(x = 0.7, y = 15, 
                label = "*s* = 6.48 (3.68, 6.65)*** <br> *h* = 0.82 (0.72, 0.89)***"), 
            size = 8, hjust = 0, vjust = 1, 
            fill = NA, label.color = NA) +
...
R3_ld50 = ggplot() +
  ...
  # Add the s and h coefficients for R3
   geom_richtext(aes(x = 0.7, y = 15, 
                label = "*s* = 9.09 (5.18, 9.10)*** <br> *h* = 0.81 (0.70, 0.89)***"), 
            size = 8, hjust = 0, vjust = 1, 
            fill = NA, label.color = NA) +
...
```{r}
all.metrics.LD90 = all.metrics.ld90 %>%
  unite(col = "Parameter", Genotype.Allele, Metric, sep = "_") %>%
  rename(LD90 = Estimate) %>%
  
  mutate(LD90.SS = all.metrics.ld90 %>% filter(Genotype.Allele == "SS" & Metric == "ED10") %>% pull(Estimate)) %>%

  # Then calculate the change
  mutate(change = (LD90 - LD90.SS)/LD90.SS * 100)
```

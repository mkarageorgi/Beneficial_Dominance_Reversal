...
variance_data = DEST.resist.data.space %>%
  ...
  mutate(snp_type = ifelse(snp_id == "AChE_I161V", "I161V", "Matched SNPs")) %>%
  # relevel the snp_type factor
  mutate(snp_type = fct_relevel(snp_type, "I161V", "Matched SNPs"))
...
variance_histogram = ggplot(snp_variances, aes(x = variance_adj, fill = snp_type)) +
  ...
  scale_x_log10() +
  scale_fill_manual(values = c("I161V" = "#faa41a", "Matched SNPs" = "grey30")) +
...
cumulative_changes_annotated = cumulative_changes_filtered %>%
  mutate(
    snp_type = ifelse(snp_id == "AChE_I161V", "I161V", "Matched SNPs"),
    snp_type = fct_relevel(snp_type, "I161V", "Matched SNPs")
  )
```
